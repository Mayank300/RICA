...
export default class MyDonationScreen extends Component {
       ...
       rightElement={
           <TouchableOpacity
            style={[
              styles.button,
              {
                backgroundColor : item.request_status === "Item Sent" ? "#f74a05" : "#0cb6fa"
              }
            ]}
            onPress = {()=>{
              this.sendBook(item)
           ...
           >
             <Text style={{color:'#ffff'}}>{
               item.request_status === "Item Sent" ? "Product Sent" : "Send Product"
             }</Text>
           </TouchableOpacity>
         }
       bottomDivider
   ...
   render(){
     return(
       <View style={{flex:1}}>
         <MyHeader navigation={this.props.navigation} title="SELL PRODUCT"/>
         <View style={{flex:1}}>
           {
             this.state.allDonations.length === 0
             ?(
               <View style={styles.subtitle}>
                 <Text style={{ fontSize: 20}}>No Products Yet!</Text>
               </View>
             )
             :(
               <FlatList
